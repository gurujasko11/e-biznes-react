...
function containsObject(obj, list) {
    ...
    for (i = 0; i < list.length; i++) {
        if (list[i].category_id === obj.category_id) {
            return true;
        }
    }
...
class Home extends Component{
    ...
    getRelatedCategoryName(categories, product) {
        ...
        {
            if(categories[idx].category_id === product.category_id)
            {
        ...
        if(this.state.products.length > 0) {
            if(this.props.isAdmin){
                itemList = products.map(item => {
                    let product_link = "/product/" + item.product_id
                    return (
                        ...
                        <div className="card" key={item.product_id}>
                            <div className="card-image">
                                ...
                                <span to="/" className="btn-floating halfway-fab waves-effect waves-light blue"
                                      onClick={() => {
                                          this.handleClickNotLogin(item, item.product_id)
                    ...
                    return (
                        <div className="card" key={item.product_id}>
                            <div className="card-image">
                                ...
                                <span to="/" className="btn-floating halfway-fab waves-effect waves-light blue"
                                      onClick={() => {
                                          this.handleClick(item, item.product_id)
                                      }}><i className="material-icons">add</i></span>
                            </div>
