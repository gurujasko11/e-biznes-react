...
class DeleteProduct extends Component {
    ...
    getRelatedCategoryName(categories, product) {
        for (var idx = 0; idx < categories.length; idx++) {
            if (categories[idx].category_id === parseInt(product.category_id)) {
    ...
    render() {
        ...
        if (this.state.deletedProductId !== 0) {
            var index = products.findIndex(obj => obj.product_id === this.state.deletedProductId);
            ...
            return (
                <div className="card" key={item.product_id}>
                    <div className="card-image">
                        ...
                        <a onClick={() => this.handleDeleteAction(item.product_id)}
